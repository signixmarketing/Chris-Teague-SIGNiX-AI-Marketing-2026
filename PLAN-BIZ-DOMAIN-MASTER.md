# Plan Biz Domain Master — Implementation Order

**Status:** Complete. The three plans in this master (Vehicles, Contacts, Deals) have been implemented and are part of the current codebase.
...
**Knowledge:** **KNOWLEDGE-DOCUMENT-CENTRIC-APPS.md** describes the general pattern (products, customers, deals); **KNOWLEDGE-LEASE-JETPACKS.md** describes this application’s domain (vehicle leasing, jet packs). The design and plans implement that pattern for this codebase.
...
**Source of truth:** The individual plan files (PLAN-ADD-VEHICLES.md, PLAN-ADD-CONTACTS.md, PLAN-ADD-DEALS.md). Refer to them for model fields, seed data, URLs, and batch steps.

---

## Prerequisites

- **PLAN-BASELINE** (PLAN-MASTER plan 1) is implemented: Django project, users app with LeaseOfficerProfile, auth (login/logout), base templates, profile view/edit, initial user, and Admin. The biz domain plans add new apps under `apps/` and extend the sidebar; they do not replace baseline.

---

## 1. PLAN-ADD-VEHICLES.md
...
**Implement:** Batch 1 (app, model, migrations, seed command); Batch 2 (forms, views, URLs, templates, sidebar, optional admin). See PLAN-ADD-VEHICLES.md Section 6 and Section 6a.

---

## 2. PLAN-ADD-CONTACTS.md
...
**Implement:** Batch 1 (app, model, migrations, seed command); Batch 2 (forms, views, URLs, templates, sidebar, optional admin). See PLAN-ADD-CONTACTS.md Section 6 and Section 6a.
...
**Dependencies:** Plan 1 (Vehicles) and Plan 2 (Contacts)—Deal has M2M to Vehicle and Contact. PLAN-BASELINE provides User and base templates.

**Implement:** Batch 1 (app, DealType and Deal models, migrations with DealType seed); Batch 2 (forms with vehicles/contacts multi-select, views, URLs, templates, sidebar, optional admin). See PLAN-ADD-DEALS.md Section 6 and Section 6a.
...
|-------|------|------------------|
| 1 | PLAN-ADD-VEHICLES.md | apps.vehicles, Vehicle model (sku, year, jpin), CRUD + list, seed jet pack, sidebar |
| 2 | PLAN-ADD-CONTACTS.md | apps.contacts, Contact model (first_name, middle_name, last_name, email, phone_number), CRUD + list, seed contact, sidebar |
| 3 | PLAN-ADD-DEALS.md | apps.deals, DealType + Deal (lease_officer, deal_type, vehicles M2M, contacts M2M, properties), View/Edit split, CRUD, sidebar |
...
## Relation to PLAN-MASTER

**PLAN-MASTER** treats this group as a single step (plan 2 in the main sequence). Implement PLAN-BASELINE first, then implement the three plans in this document in order. After PLAN-BIZ-DOMAIN-MASTER, proceed to PLAN-ADD-IMAGES (PLAN-MASTER plan 3) and PLAN-DATA-INTERFACE (PLAN-MASTER plan 4).

---

*To implement the core business domain: ensure PLAN-BASELINE is complete, then implement PLAN-ADD-VEHICLES, PLAN-ADD-CONTACTS, and PLAN-ADD-DEALS in that order, following each plan’s Section 6 and Section 6a.*
