...
This document captures the design for the **core business domain** of the lease origination application: **Vehicles**, **Contacts**, **DealType**, and **Deals**. It defines the entities, their relationships, and the shared UI and workflow conventions. Implementation follows **PLAN-BIZ-DOMAIN-MASTER.md** and the individual PLAN-ADD-VEHICLES, PLAN-ADD-CONTACTS, and PLAN-ADD-DEALS files.

**Related designs:** **DESIGN-DATA-INTERFACE.md** defines the deal-centric schema and `get_deal_data(deal)` that consume this domain. **DESIGN-DOCS.md** assumes Deals (and Deal Type) exist and uses them for document sets and signing.

**Knowledge:** **KNOWLEDGE-DOCUMENT-CENTRIC-APPS.md** describes the general pattern (products, customers, deals, document-centric flow) that this domain instantiates. **KNOWLEDGE-LEASE-JETPACKS.md** describes this application’s domain (vehicle leasing, jet packs, business objects, terminology).
...
This design assumes only **PLAN-BASELINE** is implemented (see **DESIGN-BASELINE.md** and **KNOWLEDGE-APP-FOUNDATION.md** for baseline design and capabilities):
...
Full field specs, migrations, and seed data are in the individual PLAN files (PLAN-ADD-VEHICLES, PLAN-ADD-CONTACTS, PLAN-ADD-DEALS).
...
- **Sidebar** — Each entity has a sidebar link. Order: Profile → Vehicles → Contacts → Deals → … (then Images, Schema, etc. per PLAN-MASTER). Use base template and existing layout (e.g. Jazzmin custom_links or equivalent).
...
- **Order** — Implement **Vehicles** first, then **Contacts**, then **Deals**. See **PLAN-BIZ-DOMAIN-MASTER.md** for the full sequence and deliverables.
- **Source of truth for implementation** — **PLAN-ADD-VEHICLES.md**, **PLAN-ADD-CONTACTS.md**, **PLAN-ADD-DEALS.md** (model fields, URLs, batch steps, verification).
- **DESIGN-DATA-INTERFACE.md** — Assumes this domain exists; defines deal-centric schema and `get_deal_data(deal)` using Deal, Vehicle, Contact, User.
- **DESIGN-DOCS.md** — Assumes Deals and Deal Type exist; document sets and templates are associated with deals and deal type.
...
*End of design. Implementation follows PLAN-BIZ-DOMAIN-MASTER.md and the individual PLAN-ADD-VEHICLES, PLAN-ADD-CONTACTS, and PLAN-ADD-DEALS files. See DESIGN-DATA-INTERFACE.md for schema and deal data; DESIGN-DOCS.md for documents and signing.*
