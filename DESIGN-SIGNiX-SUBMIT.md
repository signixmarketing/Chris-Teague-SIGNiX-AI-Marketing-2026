...
|--------------|-----------------|----------------|
| **CustInfo** | Sponsor, Client, UserId, Pswd, Workgroup, Demo, DelDocsAfter, EmailContent | SignixConfig (admin UI); Section 4.4, 6.2, 7.6. |
...
| **Data** | DocSetDescription | Derived from deal and **document set template name**: e.g. `"Deal #&lt;deal_id&gt; – &lt;template_name&gt;"` or `"Deal #&lt;deal_id&gt; – &lt;deal_type&gt; – &lt;template_name&gt;"`. Template name from `document_set.document_set_template.name`, or deal type name if template name is blank. Implementation may use a configurable template. |
...
| **Data** | DeliveryType | Omit or use Flex default; if required, use a constant or config value. |
| **Data** | SuspendOnStart | Constant **false** (we send immediately; no suspend). |
| **MemberInfo** | One per signer, in signer order | Order from stored signer order (Section 4.3, 5.3); first in list = first MemberInfo. |
...
| **MemberInfo** | SSN, DOB | Default values when not provided: **SSN** `"910000000"`, **DOB** `"1112223333"`. Contact model does not currently store SSN/DOB; use these defaults for initial auth types (SelectOneClick, SMSOneClick). Override if/when KBA or another auth source is used. |
