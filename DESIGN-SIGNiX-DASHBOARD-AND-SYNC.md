...
**Design references:** **KNOWLEDGE-SIGNiX.md** (push format, action→status mapping, DownloadDocument, ConfirmDownload, per-transaction push URL). **DESIGN-SIGNiX-SUBMIT.md** (SignatureTransaction model, dashboard and Deal View tables, submit flow; Section 8 defers push and download to this design). **DESIGN-DOCS.md** (DocumentInstanceVersion status: Draft → Submitted to SIGNiX → Final). **PLAN-NGROK.md** (ngrok tunnel and codebase changes so the app is reachable at an HTTPS URL).

**Next step:** After this design is complete, create multiple plan files and a **master plan** (e.g. PLAN-SIGNiX-DASHBOARD-SYNC-MASTER.md) that defines the order of those plans, then link that master from **PLAN-MASTER.md** so there is a clear end-to-end process for implementing dashboard, sync, and download in the correct order.
...
| Document | Use |
|----------|-----|
| **KNOWLEDGE-SIGNiX.md** | Push notification format (GET, query params: action, id, extid, pid, refid, ts); response 200 OK body "OK"; action→status mapping; idempotency; DownloadDocument, ConfirmDownload; per-transaction push URL (UseClientNotifyVersion2, TransactionClientNotifyURL). |
| **DESIGN-SIGNiX-SUBMIT.md** | SignatureTransaction model (deal, document_set, signix_document_set_id, transaction_id, status, first_signing_url, submitted_at, completed_at); dashboard list (Section 7.3) and Deal View related table (Section 7.5); submit flow and build_submit_document_body. |
| **DESIGN-DOCS.md** | Document set, DocumentInstance, DocumentInstanceVersion; status flow Draft → Submitted to SIGNiX → Final. |
| **PLAN-NGROK.md** | ngrok setup, ALLOWED_HOSTS, CSRF_TRUSTED_ORIGINS, health endpoint, NGROK_DOMAIN; codebase changes so the app is reachable at `https://<ngrok-domain>/...`. |
...
- **URL:** **`/signix/push/`** (trailing slash per Django convention). Full URL that SIGNiX will call: **`<base_url>/signix/push/`** (e.g. `https://your-ngrok-domain.ngrok-free.dev/signix/push/`).
...
- For **download on complete**: before calling DownloadDocument, check that the transaction status is Complete (or that we have not already created Final versions for this transaction). If already processed, skip download/ConfirmDownload and log.
...
- **TransactionClientNotifyURL** — The full URL of the push listener: **`<base_url>/signix/push/`**.
...
- **Request:** Include **IncludeAuditData** (true) and **AuditTrailFormat** (e.g. `pdf`) so the response includes the audit trail as a PDF. Include the optional element for **certificate of completion** so the response includes that summary PDF when supported (see KNOWLEDGE-SIGNiX and Flex API).
...
- **Mapping rule:** Match the documents returned by DownloadDocument to **document_set.instances** by **position** (first returned document = first instance in order) or by **RefID** if the Flex API returns RefID per document. Confirm the exact response structure in the Flex API documentation. If the count does not match, log an error and create versions for the instances that can be matched; do not fail the whole flow for a single mismatch if partial save is acceptable.
...
- **Idempotency:** If the download flow runs more than once (e.g. retry or manual re-download), overwriting existing files is acceptable; alternatively, only save when the field is currently empty. Document the choice in the implementation plan.
...
- **Depends on:** DESIGN-SIGNiX-SUBMIT implemented (SignatureTransaction, submit flow, dashboard and Deal View tables per Plans 1–9 in PLAN-SIGNiX-SUBMIT-MASTER). PLAN-NGROK applied (tunnel and codebase changes so the app is reachable at an HTTPS URL).
- **Next step:** Create **plan files** (e.g. one for push listener + status updates, one for SubmitDocument URL, one for download + ConfirmDownload, or a different split). Create **PLAN-SIGNiX-DASHBOARD-SYNC-MASTER.md** (or similar) that lists those plans in order and links from **PLAN-MASTER.md** so implementers follow the correct end-to-end order.
...
*End of design. Implementation will follow plan files and the dashboard/sync master plan. For Flex API and push details, see KNOWLEDGE-SIGNiX.md and the SIGNiX documentation.*
