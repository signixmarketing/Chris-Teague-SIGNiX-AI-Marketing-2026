...
- **Purpose** — For **testing**; may be removed or hidden in a later stage. Deletes **all** signature transactions in the system, **including their stored audit trail and certificate of completion files** from media storage, so no orphaned files remain.
