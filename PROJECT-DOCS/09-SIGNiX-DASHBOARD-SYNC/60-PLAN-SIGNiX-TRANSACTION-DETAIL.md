...
- **Signers table:** Rows in **signing order** (from get_signer_order_for_deal(deal, document_set_template)); columns: order, name, email, authentication method (e.g. get_signer_authentication_for_slot), signed (yes/no from signers_completed_refids or events), **signed at** (occurred_at of party_complete event for this signer, matched first by refid/pid against known local signer values and then by remaining event order).
...
- **Audit trail and certificate:** Batch 1 may render **availability state only** ("Available" / "Not available") from transaction.audit_trail_file and transaction.certificate_of_completion_file; the actual View/Download links are completed in Batch 2 when the file-serving URLs and views exist.
...
- **Serving audit trail and certificate** — Two views (e.g. `/deals/signatures/<pk>/audit-trail/` and `/deals/signatures/<pk>/certificate/`) that check authentication, return **404** when the stored file is missing, and otherwise stream the stored PDF inline using the file already attached to the transaction. DESIGN Section 8.4.
...
- **Signed:** True if this signer's refid or pid is in transaction.signers_completed_refids, or derived from party_complete events. In practice, matching may need to try multiple local candidate values (e.g. email, slot/order identifiers) before falling back to event order.
- **Signed at:** The **occurred_at** of the **party_complete** event that corresponds to this signer. Match signer position to event by refid/pid when present on the event; if those values do not correlate cleanly with local signer identifiers, fall back to order of unmatched party_complete events (first unmatched event = first unmatched signer, etc.). When not signed or no matching event, show "—".

### 2.3 Documents table

- **Rows:** transaction.document_set.instances.order_by('order') (or document set's instance order).
- **Document:** Prefer the source template identifier (e.g. ref_id); fall back to template description; then "—" when the source template is missing.
...
- **Data source:** transaction.events.order_by("occurred_at", "pk") so ties on timestamp still render deterministically.
...
- When transaction.audit_trail_file: show link "View audit trail" → `/deals/signatures/<pk>/audit-trail/`. When not set: "Audit trail not available" or hide.
- When transaction.certificate_of_completion_file: show link "View certificate" → `/deals/signatures/<pk>/certificate/`. When not set: "Certificate not available" or hide.
...
- **View logic** — Resolve SignatureTransaction by pk; require login; if transaction.audit_trail_file (or certificate_of_completion_file) is empty, return 404. Stream the file with `FileResponse(..., content_type="application/pdf", as_attachment=False)` so the PDF opens inline. Preserve the stored filename when available (fall back to `audit_trail.pdf` / `certificate_of_completion.pdf` only when needed).
- **Permission** — In this app, use the same rule as the signature transaction list and document PDF views: `login_required`.
...
2. **View signature_transaction_detail** — Get SignatureTransaction by pk (or 404). Check user is authenticated. Build context:
  ...
  - **Signers:** Use a **helper get_signers_detail_for_transaction(transaction)** in `apps.deals.signix` (or a small transaction_detail module) that returns a list of dicts: for each slot in get_signer_order_for_deal(deal, document_set.document_set_template), resolve name/email (resolve_signer_slot), authentication (get_signer_authentication_for_slot), signed (from signers_completed_refids or party_complete events), signed_at (occurred_at of matching party_complete event). Matching should first try explicit event refid/pid against local signer values that are actually available in-app (email, slot/order identifiers, etc.), then fall back to the next unmatched party_complete event by order. This keeps the view thin and the logic testable.
  ...
  - **Events:** transaction.events.order_by("occurred_at", "pk"); map each event_type to a human-readable label via **get_event_type_display(event_type)** in the same module (e.g. submitted→"Transaction submitted", send→"Sent", party_complete→"Signer completed", complete→"Transaction completed", etc.).
  - **Audit/certificate:** Batch 1 can expose presence/availability of audit_trail_file and certificate_of_completion_file; Batch 2 adds the file-serving links and URLs.
   Render template.

3. **Template** — Create templates/deals/signature_transaction_detail.html with sections: **header** (Transaction ID, Deal link, document set type, status, last status updated); **signers table** (columns: order, name, email, authentication, signed, signed at); **documents table** (columns: document, As sent link, Signed link or "Pending"); **events table** (columns: date/time, event label); **audit trail** and **certificate** (Batch 1: availability text is acceptable; Batch 2: replace or augment with working links).
...
7. **Views signature_transaction_audit_trail and signature_transaction_certificate** — Resolve transaction; require login; if file field empty return 404; return `FileResponse` with transaction.audit_trail_file or certificate_of_completion_file, `Content-Type: application/pdf`, and inline `Content-Disposition`. Use the stored filename when present.
...
**How to test:** Click "View audit trail" / "View certificate" from detail page; confirm PDF opens inline. For a transaction without files, confirm detail page shows "not available" and direct visit to the artifact URL returns 404. **Run unit tests (Section 6):** test_signature_transaction_audit_trail_returns_404_when_file_empty, test_signature_transaction_audit_trail_returns_pdf_when_file_set, and certificate equivalents.
...
- **test_signature_transaction_certificate_returns_pdf_when_file_set** — Same as audit trail: transaction with certificate_of_completion_file set; GET certificate URL; assert 200, Content-Type application/pdf, and inline Content-Disposition includes the filename.
...
| Signers   | **get_signers_detail_for_transaction(transaction)** (in signix or transaction_detail module): uses get_signer_order_for_deal, resolve_signer_slot, get_signer_authentication_for_slot; signed from signers_completed_refids or party_complete events; signed_at from transaction.events.filter(event_type="party_complete") matched first by refid/pid against local signer values, then by remaining event order. |
| Documents | document_set.instances.order_by('order'); label from source template ref_id or description; per instance: version with status VERSION_STATUS_SUBMITTED_TO_SIGNIX (as sent), version with status Final (signed). |
| Events    | transaction.events.order_by("occurred_at", "pk"); **get_event_type_display(event_type)** for human-readable label. |
...
- **Content-Disposition:** Implement inline so the PDF opens in the browser; preserve the stored filename in the response when possible.
- **Permission:** Detail page and file views use the same permission as signature_transaction_list and existing document PDF views: `login_required`.
- **Signed at matching:** When multiple party_complete events exist, match to signer order by refid/pid when those values can be correlated to local signer values (email, slot/order identifiers, etc.); when refid/pid is absent or ambiguous, match by order of unmatched party_complete events (first unmatched event = first unmatched signer in order). Implementation can add a helper that returns (signer_index, signed_at) per signer from transaction.events and signers_completed_refids.
