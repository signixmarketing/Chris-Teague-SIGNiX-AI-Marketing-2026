...
**Part of the business-domain sequence.** See [PHASE-PLANS-BIZ-DOMAIN.md](PHASE-PLANS-BIZ-DOMAIN.md) for implementation order and how this plan relates to Contacts and Deals. **Design:** [DESIGN-BIZ-DOMAIN.md](DESIGN-BIZ-DOMAIN.md) (entity model, relationships, UI conventions).
...
- **Persistence**: Vehicle data is stored in the project’s existing **SQLite** database (same as in the main ../01-BASELINE/10-PLAN-BASELINE.md). No additional database or configuration is required; Django migrations create the table and the ORM handles reads/writes.
