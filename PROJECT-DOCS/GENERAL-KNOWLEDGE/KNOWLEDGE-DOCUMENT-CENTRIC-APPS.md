...
3. **File assets (images)** — Uploaded images (or other assets) are stored with stable URLs or identifiers so templates can reference them (e.g. logos). See **../03-IMAGES/DESIGN-IMAGES.md** and **KNOWLEDGE-FILE-ASSETS-MEDIA.md**.
4. **HTML-to-PDF** — Rendered HTML from dynamic templates is converted to PDF (e.g. wkhtmltopdf + pdfkit) before storage. See **../05-SETUP-WKHTMLTOPDF/SETUP-WKHTMLTOPDF.md** and **KNOWLEDGE-HTML-TO-PDF.md**.
5. **Document templates** — The organization defines **static** templates (e.g. PDFs with form fields) and **dynamic** templates (e.g. HTML with placeholders mapped to deal data). **Document set templates** say “for this deal type, generate these documents in this order.” See **../06-DOCS/DESIGN-DOCS.md**.
...
| Document | Content |
|----------|---------|
| **../01-BASELINE/DESIGN-BASELINE.md** | Application baseline: auth, profile (with timezone), app shell, admin integration, initial user. |
| **../02-BIZ-DOMAIN/DESIGN-BIZ-DOMAIN.md** | This application’s core domain design (Vehicles, Contacts, Deals, DealType) and UI conventions. |
| **../04-DATA-INTERFACE/DESIGN-DATA-INTERFACE.md** | Deal-centric schema and `get_deal_data(deal)` for document population and debugging. |
| **../06-DOCS/DESIGN-DOCS.md** | Document templates, document sets, and workflow toward signing. |
| **../03-IMAGES/DESIGN-IMAGES.md** | Image (file asset) upload, stable URLs, how document features consume images; extension point for DMS. |
| **../07-SIGNiX-SUBMIT/DESIGN-SIGNiX-SUBMIT.md** | Submitting document sets for signature (SIGNiX). |
| **../09-SIGNiX-DASHBOARD-SYNC/DESIGN-SIGNiX-DASHBOARD-AND-SYNC.md** | Push, status sync, download, and transaction detail. |
...
| **../02-BIZ-DOMAIN/KNOWLEDGE-LEASE-JETPACKS.md** | This application’s domain: vehicle leasing (jet packs), business objects, and terminology. |
...
| **../70-PLAN-MASTER.md** | Implementation order for the full stack; PHASE-PLANS-BIZ-DOMAIN for the core domain. Document generation enablers (plans 3–4 + SETUP-WKHTMLTOPDF): DESIGN-DATA-INTERFACE, DESIGN-IMAGES, SETUP-WKHTMLTOPDF; knowledge in KNOWLEDGE-FILE-ASSETS-MEDIA and KNOWLEDGE-HTML-TO-PDF. |
| **../05-SETUP-WKHTMLTOPDF/SETUP-WKHTMLTOPDF.md** | Setup for wkhtmltopdf + pdfkit before dynamic document generation; run after plans 1–4, before PHASE-PLANS-DOCS. See KNOWLEDGE-HTML-TO-PDF.md for background. |

---

*This knowledge file describes the **pattern** of document-centric business applications. For the concrete business objects and terminology in the lease/jet-pack application, see **../02-BIZ-DOMAIN/KNOWLEDGE-LEASE-JETPACKS.md**.*
