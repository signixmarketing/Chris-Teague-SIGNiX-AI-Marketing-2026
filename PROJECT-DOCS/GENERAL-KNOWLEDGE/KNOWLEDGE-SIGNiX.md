...
| [Text Tagging (Flex API)](https://www.signix.com/api-text-tagging) | Using text anchors (tags) in PDFs to place signature, date, text, checkbox, and notary seal fields without AcroForm fields; coordinate system and sample SubmitDocument snippets. **Practice:** Use unique anchor text per field per document; if not possible, update the template or use white text tagging (same color as background so signer doesn't see the tag). |
