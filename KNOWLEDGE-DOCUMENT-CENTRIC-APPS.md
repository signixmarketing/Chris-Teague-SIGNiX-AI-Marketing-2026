...
2. **Schema and deal data** — A **data interface** (e.g. `get_deal_data(deal)`) returns the full deal-centric structure (deal properties + linked products + linked people + officer) in a consistent, JSON-friendly shape. Document templates use this as the **single source** for populating documents. See **DESIGN-DATA-INTERFACE.md** in this repo.
3. **File assets (images)** — Uploaded images (or other assets) are stored with stable URLs or identifiers so templates can reference them (e.g. logos). See **DESIGN-IMAGES.md** and **KNOWLEDGE-FILE-ASSETS-MEDIA.md**.
4. **HTML-to-PDF** — Rendered HTML from dynamic templates is converted to PDF (e.g. wkhtmltopdf + pdfkit) before storage. See **SETUP-WKHTMLTOPDF.md** and **KNOWLEDGE-HTML-TO-PDF.md**.
5. **Document templates** — The organization defines **static** templates (e.g. PDFs with form fields) and **dynamic** templates (e.g. HTML with placeholders mapped to deal data). **Document set templates** say “for this deal type, generate these documents in this order.” See **DESIGN-DOCS.md**.
...
7. **Signing** — When ready, the employee sends the document set for signature. Signers (derived from the deal’s people and from template configuration) receive the documents and sign electronically. The app tracks status and, when complete, stores the signed documents. See **DESIGN-SIGNiX-SUBMIT.md**, **DESIGN-SIGNiX-DASHBOARD-AND-SYNC.md**, and **KNOWLEDGE-SIGNiX.md** in this repo.
...
| Document | Content |
|----------|---------|
| **DESIGN-BASELINE.md** | Application baseline: auth, profile (with timezone), app shell, admin integration, initial user. |
| **DESIGN-BIZ-DOMAIN.md** | This application’s core domain design (Vehicles, Contacts, Deals, DealType) and UI conventions. |
| **DESIGN-DATA-INTERFACE.md** | Deal-centric schema and `get_deal_data(deal)` for document population and debugging. |
| **DESIGN-DOCS.md** | Document templates, document sets, and workflow toward signing. |
| **DESIGN-IMAGES.md** | Image (file asset) upload, stable URLs, how document features consume images; extension point for DMS. |
| **DESIGN-SIGNiX-SUBMIT.md** | Submitting document sets for signature (SIGNiX). |
| **DESIGN-SIGNiX-DASHBOARD-AND-SYNC.md** | Push, status sync, download, and transaction detail. |
| **KNOWLEDGE-APP-FOUNDATION.md** | Baseline capabilities (auth, profile with timezone, app shell, admin, initial user); technology-agnostic; Django implementation in PLAN-BASELINE. |
| **KNOWLEDGE-FILE-ASSETS-MEDIA.md** | File upload and storage without a DMS; stable URLs; when to use a DMS. |
| **KNOWLEDGE-LEASE-JETPACKS.md** | This application’s domain: vehicle leasing (jet packs), business objects, and terminology. |
| **KNOWLEDGE-HTML-TO-PDF.md** | HTML-to-PDF for document generation; wkhtmltopdf and pdfkit; constraints and alternatives. |
| **KNOWLEDGE-SIGNiX.md** | SIGNiX Flex API and integration reference. |
| **PLAN-MASTER.md** | Implementation order for the full stack; PLAN-BIZ-DOMAIN-MASTER for the core domain. Document generation enablers (plans 3–4 + SETUP-WKHTMLTOPDF): DESIGN-DATA-INTERFACE, DESIGN-IMAGES, SETUP-WKHTMLTOPDF; knowledge in KNOWLEDGE-FILE-ASSETS-MEDIA and KNOWLEDGE-HTML-TO-PDF. |
| **SETUP-WKHTMLTOPDF.md** | Setup for wkhtmltopdf + pdfkit before dynamic document generation; run after plans 1–4, before PLAN-DOCS-MASTER. See KNOWLEDGE-HTML-TO-PDF.md for background. |

---

*This knowledge file describes the **pattern** of document-centric business applications. For the concrete business objects and terminology in the lease/jet-pack application, see **KNOWLEDGE-LEASE-JETPACKS.md**.*
