...
This document captures the design for **image** (file asset) upload, storage, and exposure in the lease application. Images are used in document templates (e.g. logos, diagrams); they are stored by the application under a media root and referenced by a **stable URL** or **stable identifier** (e.g. UUID) so that template references survive file replacement. Implementation follows **PLAN-ADD-IMAGES.md**.

**Related designs:** **DESIGN-DOCS.md** defines how dynamic document templates consume images (image variables, mapping optgroup, `image:<uuid>`, context builder resolution). **DESIGN-DATA-INTERFACE.md** defines deal data; images are a **separate** source for the mapping UI (Images optgroup), not part of the deal-centric schema.

**Knowledge:** **KNOWLEDGE-FILE-ASSETS-MEDIA.md** describes the general pattern of file upload and storage without a document management subsystem, stable URLs, and when to use a DMS. This design implements that pattern for images.
...
This design assumes **PLAN-BASELINE** and **PLAN-BIZ-DOMAIN-MASTER** (Vehicles, Contacts, Deals) are implemented:
...
Full field specs and upload_to behavior are in PLAN-ADD-IMAGES.md.
...
- **Implementation:** **PLAN-ADD-IMAGES.md** — Batch 1 (app, media config, model, migrations), Batch 2 (forms, views, URLs, templates, sidebar, optional admin). Section 6 and Section 6a for order and verification.
- **PLAN-MASTER:** Images is plan 3 in the main sequence (after Biz Domain Master, before Data Interface). Document features (PLAN-DOCS-MASTER) assume Images and Data Interface are in place.
...
*End of design. Implementation follows PLAN-ADD-IMAGES.md. See DESIGN-DOCS.md for image variables and mapping; KNOWLEDGE-FILE-ASSETS-MEDIA.md for the file-asset pattern.*
