...
- **Products, customers, and deals** (PLAN-BIZ-DOMAIN-MASTER) — Need authenticated users, a “current user,” and a consistent app shell (sidebar, content area) so that list/add/edit/delete pages share the same layout and navigation.
...
- **Document templates and document sets** (PLAN-DOCS-MASTER) — Need deal data, which is tied to users and profiles; profile timezone drives how dates and times are displayed app-wide.
- **Signing** (PLAN-SIGNiX-SUBMIT-MASTER, etc.) — Need the same shell (Documents section on deal detail, Send for Signature) and the same notion of “current user” and profile.
...
The same **baseline capabilities** can be implemented in other stacks (see Section 6). This repo provides the **Django** implementation in PLAN-BASELINE.md; DESIGN-BASELINE.md states the decisions any implementation should satisfy.
...
- Use **this knowledge file** and **DESIGN-BASELINE.md** as the **capability and design checklist**. They describe *what* the baseline must provide and *why*, not how to code it in Django.
- **PLAN-BASELINE.md** remains the **Django** implementation reference. You do not need to duplicate it in another language; the value for a non-Python project is the checklist (auth, profile with timezone, app shell, admin, initial user) and the design decisions (root redirect, logout via POST, two layouts, etc.).
...
| Document | Content |
|----------|---------|
| **DESIGN-BASELINE.md** | Design decisions for the baseline (profile shape, redirect, logout, shell, admin, initial user). |
| **PLAN-BASELINE.md** | Django implementation: project structure, users app, profile, auth, base templates, admin, setup command. Section 13 = order, Section 14 = batches and verification. |
| **PLAN-MASTER.md** | Baseline is plan 1; then biz domain, images, data interface, wkhtmltopdf, document features, signing. |
...
*This knowledge file describes the **foundation** for data-centric and document-centric business applications. For design decisions, see **DESIGN-BASELINE.md**. For the Django implementation, see **PLAN-BASELINE.md**.*
