...
**Design context:** DESIGN-SIGNiX-SUBMIT.md Section 8 defers push notifications and notes that **ngrok** (or similar) can expose a local URL so SIGNiX can POST webhooks to the Django app. This plan enables that. KNOWLEDGE-SIGNiX.md and [SIGNiX Push Notifications](https://www.signix.com/pndocumentation) describe the webhook format and subscription.
...
- In **build_document_context**, when building the base URL for image variables, **prefer** `settings.PDF_IMAGE_BASE_URL` when set, so image URLs are always local and wkhtmltopdf can fetch them. Then build image URLs from that base first; fall back to `request.build_absolute_uri` or `SITE_URL` only when `PDF_IMAGE_BASE_URL` is not set. (See PLAN-NGROK implementation notes or the current repo for the exact logic.)
...
- DESIGN-SIGNiX-SUBMIT.md — Section 8 (Push notifications deferred; ngrok for dev)
- KNOWLEDGE-SIGNiX.md — Push notifications overview
